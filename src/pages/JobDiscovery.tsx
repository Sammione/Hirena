...
export default function JobDiscovery() {
    ...
    const [lastOptimized, setLastOptimized] = useState<any>(null);
    ...
    const handleOptimizeCV = async (job: Job) => {
        ...
        try {
            ...
            const userData = {
                ...
                links: profile?.experience?.[0]?.company ? `LinkedIn / GitHub` : 'Professional Profile'
            };

            const jobDescription = `${job.title} at ${job.company}. ${job.description}`;
            const result = await optimizeCVForJob(userCV, jobDescription, userData);

            setLastOptimized({ ...result, jobCompany: job.company });

            // Automatically trigger high-quality PDF generation
            generateProfessionalPDF(result.markdown, job.company, userData.full_name || 'Professional');
    ...
    const generateProfessionalPDF = (markdown: string, company: string, name: string) => {
        const doc = new jsPDF();
        let yPos = 20;
        ...
        doc.setFontSize(22);
        doc.setTextColor(0, 102, 153); // Primary Blue
        ...
        doc.text(`Optimized for ${company} • Generated by Hirena AI`, 105, yPos, { align: "center" });
        ...
        sections.forEach((section, idx) => {
            ...
            if (idx === 0) {
                // Intro/Summary (no explicit ## often at top)
                doc.setFont("helvetica", "normal");
                doc.setFontSize(11);
                doc.setTextColor(0);
                const splitDesc = doc.splitTextToSize(section.trim(), 170);
                doc.text(splitDesc, 20, yPos);
                yPos += (splitDesc.length * 6) + 12;
            } else {
                // Section Title
                doc.setFont("helvetica", "bold");
                doc.setFontSize(13);
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="fixed bottom-8 right-8 z-50 animate-in slide-in-from-right-4 duration-300">
                    <div className="bg-slate-900 border border-brand-emerald-500/30 rounded-2xl p-6 shadow-2xl flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-xs font-medium">LaTeX code copied & PDF downloaded.</p>
                        </div>
            ...
            {lastOptimized && (
                <div className="card bg-white border-2 border-brand-emerald-500/20 p-8 animate-in zoom-in-95 duration-500">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            <Sparkles className="w-6 h-6 text-brand-emerald-500" />
                            <h2 className="text-xl font-bold text-slate-900">Review Latest Optimization</h2>
                        </div>
                        <div className="flex gap-2">
                            <button
                                onClick={() => generateProfessionalPDF(lastOptimized.markdown, lastOptimized.jobCompany, lastOptimized.userData?.full_name || 'Professional')}
                                className="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl font-bold text-xs hover:bg-slate-200 transition-all"
                            >
                                Redownload PDF
                            </button>
                            <button onClick={() => setLastOptimized(null)} className="p-2 text-slate-400 hover:text-slate-600">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Content Preview</h3>
                            <div className="bg-slate-50 rounded-2xl p-6 h-[400px] overflow-y-auto text-sm text-slate-600 leading-relaxed custom-scrollbar">
                                <div className="prose prose-slate prose-sm max-w-none">
                                    {lastOptimized.markdown.split('\n').map((line: string, i: number) => (
                                        <p key={i} className={line.startsWith('##') ? 'text-lg font-bold text-slate-900 mt-4 mb-2' : ''}>
                                            {line}
                                        </p>
                                    ))}
                                </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">LaTeX Source</h3>
                                <button
                                    onClick={() => {
                                        navigator.clipboard.writeText(lastOptimized.latex);
                                        setCopied(true);
                                        setTimeout(() => setCopied(false), 2000);
                                    }}
                                    className="text-[10px] font-black uppercase text-brand-emerald-600 hover:text-brand-emerald-700 font-bold"
                                >
                                    {copied ? 'Copied!' : 'Copy LaTeX'}
                                </button>
                            </div>
                            <div className="bg-slate-900 rounded-2xl p-6 h-[400px] overflow-y-auto font-mono text-xs text-brand-emerald-400/80 leading-relaxed custom-scrollbar">
                                <pre className="whitespace-pre-wrap">{lastOptimized.latex}</pre>
                            </div>
                        </div>
                    </div>

                    <div className="mt-8 p-6 bg-brand-blue-50 rounded-2xl border border-brand-blue-100 italic text-sm text-brand-blue-900">
                        <span className="font-bold">AI Strategy:</span> {lastOptimized.explanation}
                    </div>
                </div>
            )}
